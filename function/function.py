...
def compute_sinr(h_matrix, noise_power):
    """
    计算每个子载波的SINR（关闭TxBF）
    h_matrix: shape (2, 4, 122) 的复数矩阵
    noise_power: 噪声功率（线性值）
    """
    # 计算每个子载波上信道矩阵的Frobenius范数平方
    power_per_subcarrier = np.sum(np.abs(h_matrix)**2, axis=(0, 1))
    sinr = power_per_subcarrier / noise_power
    return sinr

def eesm_mapping(sinr_values, alpha,beta):
    """
    EESM等效SINR映射
    sinr_values: 每个子载波的SINR值（122个）
    beta: EESM参数
    """
    k = len(sinr_values)
    return -np.log(np.mean(np.exp(-sinr_values / beta)))

def fit_beta(sinr_list, actual_rates):
    """
    拟合EESM参数beta，使得等效SINR与实际速率之间的误差最小
    """
    def objective(alpha,beta):
        predicted_sinr_eff = [eesm_mapping(sinr, alpha,beta) for sinr in sinr_list]
        # 假设速率与等效SINR呈线性关系（可根据实际情况调整）
        error = np.sum((np.array(predicted_sinr_eff) - actual_rates)**2)
        return error
...
def main(alpha_init=1.0, beta_init=1.0, do_fit=True):
    ...
    print(
        "结果已保存：eesm_gamma_per_sc.npy / eesm_gamma_eff.npy / eesm_results_summary.csv / eesm_time_series_comparison.png")
